  ...
  <AutoFixHeightPage :show-footer="false">
    ...
    <template #container>
      ...
      <van-cell-group :title="t('setting.encryption.status')" inset>
        ...
        <van-cell :title="t('setting.encryption.device_verified')" :label="t('setting.encryption.device_verified_desc')">
          <template #right-icon>
      ...
      <van-cell-group :title="t('setting.encryption.settings')" inset>
        <van-cell :title="t('setting.encryption.never_send_unencrypted')" :label="t('setting.encryption.never_send_unencrypted_desc')">
          <template #right-icon>
            <van-switch v-model="neverSendUnencrypted" size="20px" />
          </template>
        </van-cell>

        <van-cell :title="t('setting.encryption.show_encryption_warnings')" :label="t('setting.encryption.show_encryption_warnings_desc')">
          <template #right-icon>
    ...
    <!-- 设备详情弹出层 -->
    <van-action-sheet v-model:show="showDeviceSheet" :title="selectedDevice?.display_name || selectedDevice?.device_id">
