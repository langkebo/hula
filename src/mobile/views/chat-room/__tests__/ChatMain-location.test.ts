...
vi.mock('#/views/chat-room/LocationShare.vue', () => ({
  default: { name: 'LocationShare', template: '<div class="mock-location-share"></div>', props: ['show', 'roomId'], emits: ['update:show'] },
}))

const i18n = createI18n({ legacy: false, locale: 'en-US', messages: { 'en-US': { mobile_chat: { import_model: 'Import' }, message_container: { long_press: { menu_title: '', reply: '', copy: '', forward: '', react: '', pin: '', delete: '', multi_select: '' }, reaction: { add_failed: '' } }, common: { cancel: '' } } } })
