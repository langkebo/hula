 ...
 *
 * 在 Vue 组件中提供对 HuLa 扩展推送规则（friend_request / widget_event / ai_tool_result）
...
export function usePushRulesExtended() {
  ...
  /**
   * 判断事件类型是否属于 HuLa 扩展事件（friend_request/widget_event/ai_tool_result）。
   * 大小写不敏感。
   *
   * @param eventType Matrix/HuLa 事件类型字符串
   */
