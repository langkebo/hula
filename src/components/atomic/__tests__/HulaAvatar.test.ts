...
import HulaAvatar from '../HulaAvatar.vue'
...
describe('HulaAvatar', () => {
  ...
  it('renders img with resolved src', () => {
    const wrapper = mount(HulaAvatar, {
  ...
  it('renders round avatar by default', () => {
    ...
    expect(wrapper.find('.hula-avatar--round').exists()).toBe(true)
  })
  ...
  it('applies size to inline style', () => {
    ...
    const style = wrapper.find('.hula-avatar').attributes('style') || ''
  ...
  it('exposes role=img for screen readers', () => {
    ...
    expect(wrapper.find('.hula-avatar').attributes('role')).toBe('img')
    expect(wrapper.find('.hula-avatar').attributes('aria-label')).toBe('User avatar')
  })
})
