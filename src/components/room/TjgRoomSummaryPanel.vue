<template>
  <div class="tjg-room-summary-panel" :class="{ 'is-empty': !roomId }">
    <template v-if="!roomId">
      <div class="pane-empty">
        <div class="pane-empty__icon-shell">
          <svg class="size-48px color-[--tjg-text-tertiary]"><use href="#view-grid-card"></use></svg>
        </div>
    ...
    <template v-else>
      ...
      <template v-else-if="hasPermission === false">
        <div class="pane-no-permission">
          <div class="pane-no-permission__icon-shell">
            <svg class="size-48px color-[--tjg-text-tertiary]"><use href="#lock"></use></svg>
          </div>
          ...
          <div class="mt-24px w-full px-20px">
            <TjgSpaceJoinCta :space-id="roomId" :membership="membership" @success="emit('refresh')" />
          </div>
        </div>
      </template>
          ...
          <div class="detail-card">
            <dl class="detail-meta">
              ...
              <div v-if="onlineCount > 0" class="detail-meta__row">
                ...
                <dd class="color-[--tjg-color-success-500]">{{ onlineCount }}</dd>
              </div>
            </dl>
          </div>
      ...
      <template v-else>
        <div class="pane-error">
          <p class="color-[--tjg-text-tertiary]">{{ t('room.detail.load_failed') }}</p>
...
import TjgSpaceJoinCta from '@/components/workbench/TjgSpaceJoinCta.vue'
