        ...
        <template v-else>
          <div v-if="joinRule === 'invite'" class="cta-message">
            <span class="cta-message__icon-shell" aria-hidden="true">
              <svg class="size-16px color-[--hula-text-tertiary] cta-message__icon"><use href="#lock"></use></svg>
            </span>
...
.cta-button {
  ...
  transition:
    transform var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease),
    box-shadow var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease),
    filter var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease);
}
...
.hula-space-join-cta.is-loading .cta-button::after {
  opacity: 0.9;
  animation: cta-shimmer 1.15s linear infinite;
}

.hula-space-join-cta.is-success .cta-button--primary,
.hula-space-join-cta.is-success .cta-button--secondary,
.hula-space-join-cta.is-success .cta-message {
  animation: cta-success-pop 0.46s var(--hula-motion-ease-standard, ease) 1;
}
...
.cta-icon {
  transition:
    transform var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease),
    opacity var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease);
}
...
.cta-message {
  ...
  transition:
    transform var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease),
    border-color var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease),
    background-color var(--hula-motion-duration-fast, 0.2s) var(--hula-motion-ease-standard, ease);
}

.hula-space-join-cta.is-invite-only .cta-message {
  background: color-mix(in srgb, var(--hula-surface-panel-muted) 88%, var(--hula-color-primary-100));
}
...
.cta-message__icon-shell {
  ...
  height: 24px;
  border-radius: 999px;
  background: color-mix(in srgb, var(--hula-border-default) 72%, transparent);
}
...
@keyframes cta-success-pop {
  ...
  45% {
    ...
    box-shadow: 0 0 0 8px color-mix(in srgb, var(--hula-color-primary-300-alpha) 20%, transparent);
  }
  100% {
    transform: scale(1);
