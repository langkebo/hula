 ...
 * 禁止在 src/ 中使用旧别名，强制统一 --tjg-* 命名空间。
 *
 * 守卫规则：
 * 1. 禁止 var(--color-primary)（旧全局别名）— 应使用 var(--tjg-color-primary-500)
 *    例外：design-tokens.css 中的 --color-primary 定义本身（不是 var() 引用）不受此规则约束
 * 2. 禁止 --fgColor-accent（组件本地别名）— 应使用 var(--tjg-color-primary-500)
 *
...
describe('CSS 命名规范 lint 守卫', () => {
  describe('禁止 var(--color-primary) 旧别名引用', () => {
    ...
    // 不匹配 var(--tjg-color-primary-500) — 因为 --tjg- 前缀阻断了匹配
    ...
    it('src/ 中不应使用 var(--color-primary) 旧别名', () => {
      ...
      expect(
        violations,
        `发现 ${violations.length} 处 var(--color-primary) 旧别名引用，应改为 var(--tjg-color-primary-500)\n${violations.map((v) => `  ${v.file}:${v.line}`).join('\n')}`
      ).toEqual([])
    })
  })
  ...
  describe('禁止 --fgColor-accent 组件本地别名', () => {
    ...
    it('src/ 中不应定义或使用 --fgColor-accent', () => {
      ...
      expect(
        violations,
        `发现 ${violations.length} 处 --fgColor-accent，应改为 var(--tjg-color-primary-500)\n${violations.map((v) => `  ${v.file}:${v.line}`).join('\n')}`
      ).toEqual([])
    })
  })
})
