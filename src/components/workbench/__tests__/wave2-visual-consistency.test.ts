 ...
 *
 * TDD vertical slices：
 * 1. ThreadPanel.vue 使用 --tjg-color-primary-500（非旧别名 --color-primary）
 * 2. Bot.vue 3px 指示条使用 --tjg-color-primary-500（非本地别名 --fgColor-accent）
...
describe('Wave 2 - 问题 4：3px 强调指示条变量命名一致性', () => {
  describe('Tracer Bullet 1：ThreadPanel.vue', () => {
    ...
    it('3px 指示条应使用 var(--tjg-color-primary-500)（非旧别名 --color-primary）', () => {
      ...
      expect(colorValue).toBe('var(--tjg-color-primary-500)')
    })
  ...
  describe('Tracer Bullet 2：Bot.vue 3px 指示条', () => {
    ...
    it('3px 指示条应使用 var(--tjg-color-primary-500)（非本地别名 --fgColor-accent）', () => {
      const borderLeftMatch = source.match(/border-left:\s*3px\s+solid\s+([^;]+);/)
      expect(borderLeftMatch, '应存在 border-left: 3px 声明').not.toBeNull()
      const colorValue = borderLeftMatch![1].trim()
      expect(colorValue).toBe('var(--tjg-color-primary-500)')
