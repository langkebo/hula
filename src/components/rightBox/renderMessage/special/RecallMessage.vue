  ...
  <main class="w-full flex-center">
    <template v-if="isGroup">
      <n-flex align="center" :size="6" v-if="fromUserUid === userUid">
        <p class="text-(12px --color-text-tertiary) select-none cursor-default">{{ message.body.content }}</p>
        <p
          v-if="canReEdit"
          class="text-(12px --color-primary) select-none cursor-pointer"
          @click="handleReEdit(message.id)">
          重新编辑
        </p>
      </n-flex>
      <span v-else class="text-12px color-[--color-text-tertiary] select-none" v-html="recallText"></span>
    </template>
    <template v-else>
      <n-flex align="center" :size="6">
        <p class="text-(12px --color-text-tertiary) select-none cursor-default">
