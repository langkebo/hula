<template>
  <!-- 公告消息 -->
  <n-flex vertical :size="0" class="h-170px bg-[--tjg-surface-muted] rounded-8px custom-shadow" :style="cardStyle">
    <n-flex class="h-34px px-12px select-none cursor-default" align="center" justify="space-between">
      <n-flex align="center" :size="8">
        ...
        <p class="text-(12px [--tjg-text-tertiary])">{{ t('components.announcementCard.title') }}</p>
      </n-flex>
    ...
    <span class="w-full h-1px bg-[--tjg-border-default]"></span>

    <p
      :class="showDetailButton ? 'line-clamp-4' : 'line-clamp-5'"
      class="flex-1 px-12px pt-12px cursor-default box-border leading-22px break-words text-(14px [--tjg-text-primary])">
      <n-highlight
        v-if="searchKeyword"
        :text="body.content"
