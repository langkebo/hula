        ...
        <div class="max-h-400px overflow-y-auto">
          <div v-for="(file, index) in fileList" :key="index" class="flex-y-center p-12px">
            ...
            <div class="flex-1 min-w-0">
              <div
                class="text-(14px [--tjg-text-primary]) whitespace-nowrap overflow-hidden text-ellipsis pb-4px"
                :title="file.name">
                {{ file.name }}
              </div>
            ...
            <div @click="removeFile(index)" class="flex-shrink-0 pl-20px cursor-pointer">
              <svg class="size-20px hover:color-[--tjg-text-tertiary] transition-colors duration-200 ease-in-out">
