<template>
  <div class="size-full bg-[--tjg-surface-elevated] select-none cursor-default">
    <!--顶部操作栏-->
    ...
    <n-flex v-else vertical justify="center" class="p-14px box-border select-none">
      <n-flex justify="space-between" align="center">
        <n-flex align="center">
          <n-flex align="center">
            <p class="text-[--tjg-text-primary]">{{ t('message.check_update.current_version') }}:</p>
            <p class="text-(12px --tjg-text-tertiary) font-500">{{ currentVersion }}</p>
          </n-flex>

          <n-flex v-if="newVersion" align="center" class="relative">
            <svg class="w-24px h-24px select-none color-[--tjg-text-disabled]">
            ...
            <span
              class="absolute top--10px right--44px p-[4px_8px] bg-[--tag-danger-bg] rounded-6px text-(12px [--tjg-color-danger-500])">
        ...
        <n-flex align="center" size="medium">
          <div v-if="newVersionTime">
            <span class="text-(12px --tjg-text-tertiary)">{{ t('message.check_update.new_release_date') }}</span>
            <span class="text-(12px --color-primary)">{{ handRelativeTime(newVersionTime) }}</span>
          </div>

          <div v-else>
            <span class="text-(12px --tjg-text-tertiary)">{{ t('message.check_update.release_date') }}</span>
      ...
      <n-flex justify="space-between" align="center" class="mb-2px">
        <p class="text-(12px --tjg-text-tertiary)">{{ t('message.check_update.log_title') }}</p>
        <n-button text @click="toggleLogVisible">
          <n-flex align="center">
            ...
            <svg
              class="w-16px h-16px select-none color-[--tjg-color-primary-500] ml-2px transition-transform duration-300"
      ...
      <div
        ...
        <n-scrollbar class="p-[0_10px] box-border">
          <div v-if="newCommitLog.length > 0">
            <div class="p-[4px_8px] mt-4px w-fit bg-[--tag-danger-bg] rounded-6px text-(12px [--tjg-color-danger-500])">
              {{ newVersion }}
            </div>
            ...
            <n-flex>
              <n-flex vertical :size="20">
                <svg class="m-[4px_40px] w-24px h-24px select-none rotate-270 color-[--tjg-text-disabled]">
                  <use href="#RightArrow"></use>
                </svg>

                <span class="p-[4px_8px] w-fit bg-[--tag-neutral-bg] rounded-6px text-(12px [--tjg-text-tertiary])">
                  {{ currentVersion }}
                </span>
              </n-flex>
            </n-flex>
