<template>
  <div class="flex items-center justify-center p-[20px_0]">
    <div class="flex items-center justify-center gap-8px">
      ...
      <p class="text-(12px [--color-text-tertiary])">{{ t('setting.footer.like_product') }}</p>
      <n-popover v-model:show="isStar" style="padding: 0" trigger="hover" placement="top">
        <template #trigger>
          <p class="text-(12px --color-primary) cursor-pointer">{{ t('setting.footer.star_cta') }}</p>
        </template>
        <n-flex vertical class="w-360px h-fit">
          ...
          <n-flex vertical :size="10" class="p-14px">
            ...
            <n-flex :size="10" class="ml-auto">
              <div
                @click="isStar = false"
                class="border-(1px solid [--color-text-quaternary]) cursor-pointer w-40px h-30px rounded-8px flex-center text-(12px [--text-color])">
      ...
      <p class="text-(12px [--color-text-tertiary])">{{ t('setting.footer.share_joiner') }}</p>
      <n-popover v-model:show="isIssue" style="padding: 0" trigger="hover" placement="top-end">
        <template #trigger>
          <p class="text-(12px --color-primary) cursor-pointer">{{ t('setting.footer.share_cta') }}</p>
        </template>
        <n-flex vertical class="w-360px h-fit">
          <video
