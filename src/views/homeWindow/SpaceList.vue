  ...
  <div class="space-list-container">
    <div class="list-header">
      <n-tabs v-model:value="activeTab" type="line" justify-content="space-evenly">
        <n-tab-pane name="spaces" :tab="t('space.tabs.spaces')">
          <template #tab>
            <div class="tab-label">
              <Icon icon="mdi:folder-multiple" :width="18" />
              <span>{{ t('space.tabs.spaces') }}</span>
              <n-badge v-if="spaceCount > 0" :value="spaceCount" :max="99" type="info" />
            </div>
          </template>
        </n-tab-pane>
        <n-tab-pane name="rooms" :tab="t('space.tabs.rooms')">
          <template #tab>
            <div class="tab-label">
              <Icon icon="mdi:chat" :width="18" />
              <span>{{ t('space.tabs.rooms') }}</span>
      ...
      <div class="header-actions">
        <n-tooltip trigger="hover">
          ...
          {{ activeTab === 'spaces' ? t('space.add_space') : t('space.add_room') }}
        </n-tooltip>
      </div>
    </div>
    ...
    <div class="list-content">
      <n-spin :show="loading">
        <n-scrollbar style="max-height: calc(100vh - 200px)">
          <div v-if="activeTab === 'spaces'" class="space-items">
            <n-collapse v-if="spaces.length > 0" :default-expanded-names="['all']">
              <n-collapse-item name="all" :title="t('space.groups.all_spaces')">
                <SpaceItem
            ...
            <n-empty v-else :description="t('space.empty')" />
          </div>

          <div v-else class="room-items">
            <n-collapse v-if="groupedRooms.size > 0" :default-expanded-names="['recent', 'groups']">
              <n-collapse-item v-if="recentRooms.length > 0" name="recent" :title="t('space.groups.recent')">
                <RoomListItem
              ...
              <n-collapse-item v-if="groupRooms.length > 0" name="groups" :title="t('space.groups.groups')">
                <RoomListItem
              ...
              <n-collapse-item v-if="directRooms.length > 0" name="direct" :title="t('space.groups.direct')">
                <RoomListItem
            ...
            <n-empty v-else :description="t('space.empty_rooms')" />
          </div>
        </n-scrollbar>
      </n-spin>
    </div>
...
const logger = createLogger('SpaceList')
const { t } = useI18n()
...
const contextMenuOptions = computed(() => {
  ...
  if (isSpace) {
    return [
      { label: t('space.context.open'), key: 'open' },
      { label: t('space.context.settings'), key: 'settings' },
      { type: 'divider', key: 'd1' },
      { label: t('space.context.leave'), key: 'leave' }
    ]
  }

  return [
    { label: t('space.context.open'), key: 'open' },
    { label: t('space.context.info'), key: 'info' },
    { type: 'divider', key: 'd1' },
    { label: t('space.context.mute'), key: 'mute' },
    { label: t('space.context.pin'), key: 'pin' },
    { type: 'divider', key: 'd2' },
    { label: t('space.context.leave'), key: 'leave' }
  ]
})
...
async function handleContextMenuSelect(key: string) {
  ...
  switch (key) {
    case 'open':
      ...
      break
    case 'info':
      message.info(t('space.context.info_opened'))
      break
    case 'settings':
      router.push({ name: 'spaceDetail', params: { roomId: target.roomId }, query: { edit: 'true' } })
      break
    case 'mute':
      message.info(t('space.context.mute_success'))
      break
    case 'pin':
      message.info(t('space.context.pin_success'))
      break
    case 'leave':
      message.info(t('space.context.leave_success'))
      break
  }
}
