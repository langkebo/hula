...
export const useFileStore = defineStore(StoresEnum.FILE, () => {
  // ==================== 状态定义 ====================

  /** 所有房间的文件数据 Map<roomId, Map<fileId, FileInfo>> */
  const roomFilesMap = shallowReactive<Record<string, Record<string, FileInfo>>>({})
  ...
  const addFile = (fileInfo: FileInfo) => {
    ...
    roomFilesMap[roomId] = { ...roomFilesMap[roomId], [id]: fileInfo }
  }
  ...
  const removeFile = (roomId: string, fileId: string) => {
    if (roomFilesMap[roomId]?.[fileId]) {
      const { [fileId]: _removed, ...rest } = roomFilesMap[roomId]
      roomFilesMap[roomId] = rest
    }
  }
