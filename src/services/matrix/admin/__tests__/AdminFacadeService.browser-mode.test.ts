...
// 预期：浏览器 dev 模式（无 Tauri runtime）下，AdminManager.getUser 返回 admin=true 时，
...
describe('AdminFacadeService — C-1 浏览器 dev 模式（无 Tauri runtime）', () => {
  ...
  it('浏览器模式下 admin API 返回 admin=true → 应执行管理员操作', async () => {
    mockAdminManager.getUser.mockResolvedValue({ name: 'admin1', admin: true })
  ...
  it('浏览器模式下 admin API 返回 admin=false → 应拒绝访问 SDK AdminManager', async () => {
    mockAdminManager.getUser.mockResolvedValue({ name: 'user1', admin: false })
  ...
  it('浏览器模式下 admin API 抛错 → 应视为非管理员并拒绝访问 SDK AdminManager', async () => {
    mockAdminManager.getUser.mockRejectedValue(new Error('403 Forbidden'))
  ...
  it('浏览器模式下 admin API 返回 admin=true 后应缓存结果（2 分钟内）', async () => {
    ...
    expect(mockAdminManager.getUser).toHaveBeenCalledTimes(1)
  ...
  it('浏览器模式下 admin 用户查询委托 AdminManager.getUser（v2）', async () => {
    mockAdminManager.getUser.mockResolvedValue({ name: 'admin1', admin: true })

    await adminService.getServerStats()

    expect(mockAdminManager.getUser).toHaveBeenCalledWith('@admin1:matrix.test')
  })
  ...
  it('浏览器模式下 clearAdminApiCache 后应重新验证', async () => {
    ...
    await adminService.getServerStats()
    expect(mockAdminManager.getUser).toHaveBeenCalledTimes(2)
  })
})
