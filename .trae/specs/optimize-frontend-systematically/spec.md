...
桌面端构建配置 SHALL 仅包含 `NaiveUiResolver`，移动端构建配置 SHALL 仅包含 `VantResolver`，消除跨平台 resolver 冗余。
...
### Requirement: 旧版服务发现层
**Reason**: `services/discovery/` 仅使用 Static 适配器，过度设计，对 IM 客户端无实际价值
**Migration**: 将 Static 配置内联到 `backend/config.ts`，删除 discovery 目录

### Requirement: 旧版错误类型（TranslatedError）
**Reason**: 与 `AppError` 功能重叠，三套错误转换机制并存导致行为不一致
**Migration**: 将 i18n key 逻辑合并到 `toAppError()`，逐步替换 `TranslatedError` 的消费者
