...
- `已完成阶段性成果`：核心开发已经落地，当前主要进入联调、验收或维护观察阶段。
- `待收尾完善`：已有部分成果，但仍存在未完成开发项、验证缺口或文档与实现不一致的问题。
- 本文档最后核对时间：`2026-05-14`

## Phase 1: 紧急修复（P0）-- 死依赖与死代码清除

- [x] Task 1: 移除未使用的 npm 依赖 `【任务状态：已完成】`
...
- [x] Task 4: 优化 App.vue 首屏加载 `【任务状态：已完成】`
...
- [x] Task 5: 修复 Worker 内存保护机制 `【任务状态：已完成】`
...
- [x] Task 9: 提取 BaseMatrixService 基类 `【任务状态：已完成】`
  - [x] SubTask 9.1: 创建 `src/services/matrix/BaseMatrixService.ts`，包含统一的 `getClient()` 和 `t()` 方法
  - [x] SubTask 9.2: 34 个服务类已继承 `BaseMatrixService`（含 i18n 迁移新增的9个），移除各自的 `getClient()` 方法
  - [x] SubTask 9.3: 运行 `vue-tsc --noEmit` 验证无类型错误（通过）

- [x] Task 10: 统一 Admin 服务架构 `【任务状态：已完成】`
...
- [x] Task 11: 修复 Store 层状态冗余 `【任务状态：已完成】`
...
- [x] Task 12: 修复 MatrixRuntimeSessionService 分层违规 `【任务状态：已完成】`
...
- [x] Task 13: 修复暗色模式硬编码颜色 `【任务状态：已完成】`
  ...
  - [x] SubTask 13.5: 修复5处JS三元判断颜色模式 + 13处桌面端硬编码颜色 + 23处桌面端组件硬编码颜色 + 72处移动端硬编码颜色，添加CSS变量（--avatar-fallback-color/--avatar-fallback-src/--voice-icon-color-other/--login-third-party-divider-color/--login-surface-bg/--plugin-icon-bg/--tag-danger-bg/--tag-neutral-bg/--tag-info-bg/--time-group-bg/--pin-input-bg/--bg-offline-avatar-overlay/--bg-long-press-menu）

- [x] Task 14: 修复国际化硬编码文本 `【任务状态：已完成】`
  ...
  - [x] SubTask 14.4: 将移动端组件中的硬编码中文迁移至 i18n（13个文件，84处硬编码中文，新增翻译文件 mobile_edit_brithday/mobile_mymessage/mobile_contact/mobile_chat/mobile_personal_info/mobile_forget_code/mobile_voice_video/mobile_qrcode/mobile_chat_setting/mobile_devices.json）
  - [x] SubTask 14.5: 修改 `i18n.ts` 语言回退策略，从中文改为英文
  - [x] SubTask 14.6: 非 robot 目录硬编码中文迁移完成 -- hooks/ 47处 + utils/ 38处 + services/matrix/ 约50处 + components/views 约30处，新增翻译文件 hooks.json/validation.json/diagnostics.json/utils.json/matrix_error.json

- [x] Task 15: 修复移动端体验问题 `【任务状态：已完成】`
  ...
  - [x] SubTask 15.3: 创建 useChatScrollManager 共享 composable（统一桌面端/移动端聊天滚动管理逻辑）
  - [x] SubTask 15.4: 修复 `App.vue` 中动态 import 样式文件缺少 `.catch()` 错误处理

## Phase 5: 长期优化（P3）-- 组件重构与完善

- [x] Task 16: 重构大型组件 `【任务状态：已完成】`
  - [x] SubTask 16.1: 拆分 `renderMessage/index.vue`（538行→257行）-- 提取 `useMessageActions` 和 `useMessageContextMenu` composable
  - [x] SubTask 16.2: 拆分 `Login.vue`（765行→111行）-- 拆分为 ManualLoginForm + AutoLoginForm + LoginBottomBar + ServerConfigModal + useLoginOrchestrator
  - [x] SubTask 16.3: 消除组件直接导入 matrix 服务层 -- P0: 清理5个Admin组件残留导入 + P1: 创建 useEncryption composable 迁移7个加密组件

- [x] Task 17: 统一 SDK 类型入口 `【任务状态：已完成】`
...
- [ ] SubTask 16.3 P2-P4: 组件直接导入 matrix 服务层 -- 已有 composable 但组件未使用（9个组件）+ 低频服务新建 composable（typing/room/account/thread/notification）+ 类型导入统一
- [ ] 将 ChatMain.vue 和 MessageContainer.vue 迁移到使用 useChatScrollManager
- [ ] 创建 useChatMessageActions / useSessionActions / useFileSendActions / useMobileChatPanel 共享 composable
