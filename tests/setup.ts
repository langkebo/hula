  ...
  useI18n: () => ({
    t: (key: string, params?: Record<string, unknown>) => {
      const translations: Record<string, string> = {
        ...
        'encryption.backup_setup_dialog.copy_success': '恢复密钥已复制到剪贴板',
        'encryption.backup_setup_dialog.download_success': '恢复密钥已下载',
        'encryption.backup_setup_dialog.copy_manual': '请手动复制密钥',
        'encryption.backup_setup_dialog.verify_success': '密钥备份验证成功',
        'encryption.backup_setup_dialog.verify_failed': '密钥备份验证失败，请稍后重试',
        'encryption.backup_setup_dialog.create_failed': '创建密钥备份失败',
