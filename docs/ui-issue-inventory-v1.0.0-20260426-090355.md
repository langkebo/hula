...
| UI-011 | 文案 | 设置窗口主壳层、搜索关键词、移动端设置中的阅后即焚时长标签、扫码页关键提示，以及部分资料编辑/AI 助手/设备与通知设置反馈文案已接入 i18n，但设置体系仍残留少量运行时日志、错误提示与辅助文案未统一，国际化收敛尚未闭环。 | 设置窗口、多语言体验 | P1 | `S-11 文案国际化断层`；代码证据: `src/views/settingsWindow/SettingsDialog.vue`、`src/composables/settings/settingsSearchIndex.ts`、`src/mobile/views/my/PreferencesSettings.vue`、`src/mobile/views/my/BurnAfterReadSettings.vue`、`src/mobile/views/my/MobileQRCode.vue`、`src/mobile/views/my/EditProfile.vue`、`src/mobile/views/my/AiAssistant.vue` | 切换语言后进入设置窗口、移动端设置页、扫码页、资料编辑页与 AI 助手，并继续核查设置运行时日志/提示是否仍保留固定文案 | 中，会影响国际化验收和文案统一治理 |
...
- 仍未解决的关键问题:
  ...
  - `UI-011`: 设置偏好页中的语言选项和发送键选项已改为消费 i18n 文案键，设置壳层的导航标题、当前标签标题与搜索索引也已接入运行时翻译来源，移动端 `PreferencesSettings.vue`、`BurnAfterReadSettings.vue` 中的阅后即焚时长标签与格式化逻辑、`MobileQRCode.vue` 中的扫码状态/权限提示、`EditProfile.vue` 中的保存反馈，以及 `AiAssistant.vue` 中的新建会话/空响应占位与建议提示词也已改为复用语言包键；但设置体系其余日志与提示文案仍需继续统一
