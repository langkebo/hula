...
- 仍未解决的关键问题:
  ...
  - `UI-004`、`UI-013`: 旧别名变量、悬空变量与设置壳层之外的样式消费点仍有进一步清理空间；`FileContent.vue`、`EmptyState.vue`、`UserList.vue`、`SideNavigation.vue`、`UserItem.vue`、`AvatarCropper.vue`、`ContextMenu.vue`、`InfoPopover.vue`、`InfoEdit.vue`、`definePlugins/List.vue`、`model.tsx`、`Login.vue`、`ManageGroupMember.vue`、`TrendRadarView.vue`、`OpenClawView.vue`、`LockScreen.vue`、`chat-header.scss`、`plugins/robot/views/chatSettings/model.tsx`、`TrendingPanel.vue`、`NewsCard.vue`、`SearchPanel.vue`、`components/openclaw/ModelSelector.vue`、`AddFriends.vue`、`ConfirmAddFriend.vue`、`ConfirmAddGroup.vue`、`SecretChatPage.vue`、`WidgetManager.vue`、`ThreadView.vue`、`ThreadIndicator.vue`、`ThreadPanel.vue`、`SpotlightDialog.vue`、`KeyBackupDialog.vue`、`KeyRotationDialog.vue`、`CrossSigningDialog.vue`、`DeviceVerifyDialog.vue` 与 `KeyBackupSetupDialog.vue` 中已确认的 `--line-color`、`--chat-text-color`、`--text-color`、`--text-color-secondary`、`--text-color-tertiary`、`--text-color-disabled`、`--text-color-3`、`--bg-primary`、`--bg-color`、`--bg-color-secondary`、`--border-color` 消费点已完成收敛
