...
| UI-011 | 文案 | 设置窗口主壳层与搜索关键词已接入 i18n，但设置体系仍残留少量运行时日志、错误提示与辅助文案未统一，国际化收敛尚未闭环。 | 设置窗口、多语言体验 | P1 | `S-11 文案国际化断层`；代码证据: `src/views/settingsWindow/SettingsDialog.vue`、`src/composables/settings/settingsSearchIndex.ts` | 切换语言后进入设置窗口，并继续核查设置运行时日志/提示是否仍保留固定文案 | 中，会影响国际化验收和文案统一治理 |
...
- 仍未解决的关键问题:
  ...
  - `UI-011`: 设置偏好页中的语言选项和发送键选项已改为消费 i18n 文案键，设置壳层的导航标题、当前标签标题与搜索索引也已接入运行时翻译来源，`SettingsDialog.vue`、`useSettingsShell.ts`、`settingsSearchIndex.ts`、`settingsSchema.ts` 与设置语言包已完成一轮联动收敛；但设置体系其余日志与提示文案仍需继续统一
