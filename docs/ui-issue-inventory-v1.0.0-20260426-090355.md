...
| UI-011 | 文案 | 设置窗口主壳层、搜索关键词、移动端设置中的阅后即焚时长标签、扫码页关键提示，以及资料编辑/AI 助手/设备与通知设置反馈文案已接入 i18n，`src/mobile/views/my` 目录中的运行时中文日志与提示也已完成一轮清理；但设置体系其他目录仍残留少量运行时日志、错误提示与辅助文案未统一，国际化收敛尚未闭环。 | 设置窗口、多语言体验 | P1 | `S-11 文案国际化断层`；代码证据: `src/views/settingsWindow/SettingsDialog.vue`、`src/composables/settings/settingsSearchIndex.ts`、`src/mobile/views/my/PreferencesSettings.vue`、`src/mobile/views/my/BurnAfterReadSettings.vue`、`src/mobile/views/my/MobileQRCode.vue`、`src/mobile/views/my/EditProfile.vue`、`src/mobile/views/my/AiAssistant.vue` | 切换语言后进入设置窗口、移动端设置页、扫码页、资料编辑页与 AI 助手，并继续核查设置体系其他目录中的运行时日志/提示是否仍保留固定文案 | 中，会影响国际化验收和文案统一治理 |
...
- 仍待正式验收或继续推进的关键事项:
  - `UI-001`、`UI-008`、`UI-010`: 设置壳层统一与旧设置隐藏依赖拆耦已完成实施，剩余工作转为正式验收与文档同步
  - `UI-002`: 运行时当前仅保留 `simple.scss` 主题入口，旧 `/versatile` 路由与 `setting.theme.versatile` 语言包分支已清理；状态层仍保留一次历史持久化数据兼容清洗逻辑，后续可结合迁移策略继续收口
  - `UI-005`、`UI-006`、`UI-009`、`UI-014`: 主工作区与房间/空间统一承载已完成实施并接入统一工作台，剩余工作转为性能、交互与验收指标验证
  - `UI-004`、`UI-013`: 旧别名变量、悬空变量与设置壳层之外的样式消费点仍有进一步清理空间；`FileContent.vue`、`EmptyState.vue`、`UserList.vue`、`SideNavigation.vue`、`UserItem.vue`、`AvatarCropper.vue`、`ContextMenu.vue`、`InfoPopover.vue`、`HomeserverDialog.vue`、`InfoEdit.vue`、`definePlugins/List.vue`、`model.tsx`、`Login.vue`、`ManageGroupMember.vue`、`TrendRadarView.vue`、`OpenClawView.vue`、`LockScreen.vue`、`chat-header.scss`、`plugins/robot/views/chatSettings/model.tsx`、`TrendingPanel.vue`、`NewsCard.vue`、`SearchPanel.vue`、`components/openclaw/ModelSelector.vue`、`AddFriends.vue`、`ConfirmAddFriend.vue`、`ConfirmAddGroup.vue`、`SecretChatPage.vue`、`WidgetManager.vue`、`ThreadView.vue`、`ThreadIndicator.vue`、`ThreadPanel.vue`、`SpotlightDialog.vue`、`CreateDmDialog.vue`、`DmListView.vue`、`FriendRequestDialog.vue`、`UrlPreviewCard.vue`、`KeyBackupDialog.vue`、`KeyRotationDialog.vue`、`CrossSigningDialog.vue`、`DeviceVerifyDialog.vue` 与 `KeyBackupSetupDialog.vue` 中已确认的 `--line-color`、`--chat-text-color`、`--text-color`、`--text-color-1`、`--text-color-secondary`、`--text-color-tertiary`、`--text-color-disabled`、`--text-color-3`、`--bg-primary`、`--bg-color`、`--bg-color-secondary`、`--border-color` 消费点已完成收敛
  - `UI-011`: 设置偏好页中的语言选项和发送键选项已改为消费 i18n 文案键，设置壳层的导航标题、当前标签标题与搜索索引也已接入运行时翻译来源，移动端 `PreferencesSettings.vue`、`BurnAfterReadSettings.vue` 中的阅后即焚时长标签与格式化逻辑、`MobileQRCode.vue` 中的扫码状态/权限提示、`EditProfile.vue` 中的保存反馈，以及 `AiAssistant.vue` 中的新建会话/空响应占位与建议提示词也已改为复用语言包键；此外 `SlidingSyncIndicator.vue`、`VirtualList.vue`、`MemoryMonitor.vue` 已接入 `components.json` 文案键，`SplashScreen.vue` 已复用 `common.retry`，`ErrorBoundary.vue` 与 `ScreenshotRoot.vue` 的中文日志/失败提示也已清理，但设置体系其余目录仍需继续统一
...
- 当前更需要做的不是继续证明方案方向，而是补齐性能、覆盖率、E2E、可访问性和国际化的正式验收，并继续收掉设置体系与低频组件中的剩余长尾问题。
