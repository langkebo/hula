...
- 已完成:
  ...
  - `src/components/common/HomeserverDialog.vue`、`src/components/dm/CreateDmDialog.vue`、`DmListView.vue`、`src/components/friend/FriendRequestDialog.vue` 与 `src/components/business/url-preview/UrlPreviewCard.vue` 已完成下一批低频弹窗与展示组件样式变量收敛，替换旧 `--text-color-1`、`--bg-color`、`--border-color` 为 `--hula-text-primary`、`--hula-surface-panel`、`--hula-border-default`；其中 `HomeserverDialog.vue` 的中文日志已同步清理，相关目录残留扫描与诊断校验均通过
  - `src/components/common/SlidingSyncIndicator.vue`、`VirtualList.vue` 与 `MemoryMonitor.vue` 已完成一批公共小组件运行时文案国际化收敛，新增并接入 `locales/zh-CN/components.json`、`locales/en/components.json` 中的 `slidingSyncIndicator.*`、`virtualList.*`、`memoryMonitor.*` 键；同时 `ErrorBoundary.vue` 的中文日志已清理，相关诊断校验均通过
  - `src/components/common/SplashScreen.vue` 与 `src/components/common/Screenshot/ScreenshotRoot.vue` 已完成一批启动页与截图链路运行时文案收敛，`SplashScreen.vue` 的重试按钮已复用 `common.retry`，`ScreenshotRoot.vue` 的中文日志与失败提示已改为复用现有 `message.screenshot.save_failed`，相关诊断校验均通过
...
- 仍待处理:
  ...
  - 运行时当前仅保留 `simple.scss` 主题入口，旧 `/versatile` 路由与 `setting.theme.versatile` 语言包分支已完成清理；状态层仍保留一次历史持久化数据兼容清洗逻辑，后续可结合迁移策略继续收口
  - 房间/空间工作区与设置壳层统一已完成实施，后续重点转为正式验收、性能指标与交互细节复核
