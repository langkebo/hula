...
| # | 差异点 | 原型 | 当前实现 | 严重度 |
|:---|:---|:---|:---|:---|
| D1 | 导航栏结构 | 8 图标（消息/好友/房间/空间/机器人/联邦/设置/管理） | 消息/好友(menuTop)+房间/空间/机器人(插件)+文件/邮件(底部)+更多菜单(设置/管理)；缺联邦独立图标 | **高** |
| D2 | 「联邦」独立入口 | 有（view-federation） | 缺（消息/好友已在 menuTop 实现，仅联邦缺失） | **高** |
| D3 | 图标语义 | 房间=i-room(拱门)、空间=i-space(立方体)、好友=i-users(多人) | ✅ 已修正：房间=room、空间=space、好友=peoples-two | ✅ 已修复 |
