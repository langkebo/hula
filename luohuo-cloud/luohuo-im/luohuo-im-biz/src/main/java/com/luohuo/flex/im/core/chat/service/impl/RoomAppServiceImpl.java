...
public class RoomAppServiceImpl implements RoomAppService, InitializingBean {
	...
	public void addAdmin(Long uid, AdminSetReq request) {
		...
		// P1优化: 清理批量缓存 (2025-12-13)
		groupMemberCache.evictMemberBatch(roomGroup.getRoomId());
		// 清理被更新的成员的个人缓存
		request.getUidList().forEach(uidItem -> groupMemberCache.evictMemberDetail(roomGroup.getRoomId(), uidItem));
	...
	public void exitGroup(Boolean isGroup, Long uid, MemberExitReq request) {
		...
		if (isGroup || isLord) {
			...
			transactionTemplate.execute(e -> {
				...
				// P1优化: 清理批量缓存 (2025-12-13)
				groupMemberCache.evictMemberBatch(roomId);
				// 4.4 删除消息记录 (逻辑删除)
