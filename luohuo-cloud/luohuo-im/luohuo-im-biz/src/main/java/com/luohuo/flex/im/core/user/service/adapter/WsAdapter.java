...
public class WsAdapter {
	 ...
	 * 构建E2EE加密消息推送
	 */
	public static <T> WsBaseResp<T> buildEncryptedMsgSend(T msgResp) {
		WsBaseResp<T> wsBaseResp = new WsBaseResp<>();
		wsBaseResp.setType(WSRespTypeEnum.E2EE_ENCRYPTED_MESSAGE.getType());
		wsBaseResp.setData(msgResp);
		return wsBaseResp;
	}
	 ...
	 * 构建E2EE会话密钥通知
	 */
	public static <T> WsBaseResp<T> buildSessionKeyNotify(T keyNotification) {
		WsBaseResp<T> wsBaseResp = new WsBaseResp<>();
		wsBaseResp.setType(WSRespTypeEnum.E2EE_SESSION_KEY.getType());
		wsBaseResp.setData(keyNotification);
		return wsBaseResp;
	}
}
