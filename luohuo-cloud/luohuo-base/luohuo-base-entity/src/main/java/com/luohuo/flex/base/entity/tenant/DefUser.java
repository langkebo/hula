...
public class DefUser extends TenantEntity<Long> {
    ...
    /**
     * 邮箱
     * ===== P0修复: PII字段加密 (2025-12-13) =====
     * 使用AES-256-GCM加密存储, 通过EncryptedStringTypeHandler自动加密/解密
     * ⚠️ 注意: 需要先在Nacos配置pii.encryption.key才能启用
     * 加密组件位置: luohuo-crypto-sdk模块 (独立SDK)
     */
    @TableField(value = "email", condition = LIKE, typeHandler = com.luohuo.flex.crypto.EncryptedStringTypeHandler.class)
     ...
     * ⚠️ 注意: 需要先在Nacos配置pii.encryption.key才能启用
     */
    @TableField(value = "mobile", condition = LIKE, typeHandler = com.luohuo.flex.crypto.EncryptedStringTypeHandler.class)
     ...
     * ⚠️ 注意: 需要先在Nacos配置pii.encryption.key才能启用
     */
    @TableField(value = "id_card", condition = LIKE, typeHandler = com.luohuo.flex.crypto.EncryptedStringTypeHandler.class)
