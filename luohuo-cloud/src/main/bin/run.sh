...
ENV_FILE="$BASE_PATH/../docs/install/docker/.env"
if [ -f "$ENV_FILE" ]; then
    set -a
    . "$ENV_FILE"
    set +a
...
JAVA_OPT="$JAVA_OPT -Dspring.profiles.active=$PROFILES -Dluohuo.version=3.0.6 -Dluohuo.nacos.namespace="
...
start() {
    ...
    if [ $count -ne 0 ]; then
        ...
        JENKINS_NODE_COOKIE=dontKillMe nohup java $JAVA_OPT -jar "$BASE_PATH/$DIRECTORY/$MODULER/target/$MODULER.jar" > "$LOG_FILE" 2>&1 &
        sleep 5
        count=$(ps -ef | grep java | grep $MODULER | grep -v grep | wc -l)
        if [ $count -ne 0 ]; then
