        ...
        for (VueRouter item : tree) {
            ...
            if (StrUtil.isNotEmpty(item.getMetaJson()) && !StrPool.BRACE.equals(item.getMetaJson())) {
                try {
                    ...
                    log.debug("Failed to parse metaJson using JsonUtil: {}", item.getMetaJson(), e);
                    try {
                        meta = objectMapper.readValue(item.getMetaJson(), RouterMeta.class);
                    } catch (Exception e2) {
                        log.warn("Failed to parse metaJson using both parsers: {}", item.getMetaJson(), e2);
                    }
                }
            }
