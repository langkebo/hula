...
import com.luohuo.flex.oauth.service.PasswordService;
...
public class PasswordTokenGranter extends AbstractTokenGranter implements TokenGranter {
    ...
    // ===== P0修复: 注入密码服务 (2025-12-13) =====
    private final PasswordService passwordService;

	public PasswordTokenGranter(SystemProperties systemProperties, DefClientService defClientService, DefUserService defUserService, BaseEmployeeService baseEmployeeService, BaseOrgService baseOrgService, SaTokenConfig saTokenConfig, ImUserApi userApi, com.luohuo.flex.oauth.biz.StpInterfaceBiz stpInterfaceBiz, PasswordService passwordService) {
    ...
    protected R<LoginResultVO> checkUserPassword(LoginParamVO loginParam, DefUser user) {
        ...
        // ===== P0修复: 安全密码验证 (2025-12-13) =====
        // 使用PasswordService验证密码 (兼容旧SHA-256和新BCrypt)
        ...
        // ===== P0修复: 自动密码升级 (2025-12-13) =====
        // 如果用户使用旧SHA-256密码, 登录成功后自动升级为BCrypt
        if (passwordService.needsPasswordUpgrade(user)) {
            try {
                log.info("自动升级用户 {} 的密码为BCrypt格式", user.getId());
                String newPasswordHash = passwordService.upgradePassword(password);
                user.setPassword(newPasswordHash);
                user.setSalt(null);  // 清除salt字段, 标记为新格式
                defUserService.updateById(user);
                log.info("用户 {} 密码升级成功", user.getId());
            } catch (Exception e) {
                // 升级失败不影响登录, 记录日志即可
                log.error("用户 {} 密码自动升级失败: {}", user.getId(), e.getMessage(), e);
            }
        }
