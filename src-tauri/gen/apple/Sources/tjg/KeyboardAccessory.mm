...
static BOOL TjgPatchTargetView(UIView *targetView) {
...
static BOOL TjgIsWKContentView(UIView *view) {
    if (!view) { return NO; }
...
static BOOL TjgPatchViewHierarchy(UIView *view) {
    if (!view) { return NO; }
    BOOL patched = NO;

    if (TjgIsWKContentView(view)) {
        patched = TjgPatchTargetView(view) || patched;
    }

    // 继续递归子视图
    for (UIView *sub in view.subviews) {
        patched = TjgPatchViewHierarchy(sub) || patched;
    }

    return patched;
}

static BOOL TjgPatchAllWindows(void) {
    BOOL patched = NO;
    if (@available(iOS 13.0, *)) {
        ...
        for (UIScene *scene in connectedScenes) {
            ...
            for (UIWindow *window in windowScene.windows) {
                ...
                patched = TjgPatchViewHierarchy(window) || patched;
            }
        }
    } else {
...
static void TjgTryPatchRepeatedly(int attempt, int maxAttempts, NSTimeInterval interval) {
    // 在主线程执行，防止与 UIKit 交互的线程问题
    dispatch_async(dispatch_get_main_queue(), ^{
        BOOL ok = TjgPatchAllWindows();
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(interval * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            TjgTryPatchRepeatedly(attempt + 1, maxAttempts, interval);
        });
    });
}
...
void tjg_disable_keyboard_accessory(void) {
    // 最多尝试 100 次，每次间隔 0.1 秒（总计约 10 秒）。
    // 一般在 WebView 创建后很快会被命中。
    TjgTryPatchRepeatedly(0, 100, 0.1);
}
